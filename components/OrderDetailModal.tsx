...
export default function OrderDetailModal({ order, onClose, onUpdateFollowUp, onEdit, onApprove, onDeny, onDelete, onDeductInventory, onGenerateInvoice }: OrderDetailModalProps) {
  ...
  const handleSendInstagram = () => {
    if (!generatedMessage) { setError("Please enter a message first."); return; }
    navigator.clipboard.writeText(generatedMessage).then(() => {
        setCopiedState('instagram');
        window.open('https://www.instagram.com/direct/inbox/', '_blank');
        setTimeout(() => setCopiedState(null), 2500);
    }).catch(err => { setError("Failed to copy message to clipboard."); console.error(err); });
  };
  ...
  const handleSendText = () => {
    if (!generatedMessage) { setError("Please enter a message first."); return; }
    if (!order.phoneNumber) { setError("Customer phone number is missing."); return; }
    const cleanedPhoneNumber = order.phoneNumber.replace(/[^0-9]/g, '');
    if (!cleanedPhoneNumber) { setError("Invalid phone number format."); return; }
          ...
          <header className={`p-6 border-b border-brand-tan flex justify-between items-center ${isPartyPlatter ? 'bg-purple-50' : ''}`}>
            ...
            <div className="flex items-center gap-2">
              {order.approvalStatus === ApprovalStatus.PENDING && onApprove && onDeny && (
                  <>
                    <button onClick={() => { onDeny(order.id); onClose(); }} className="flex items-center gap-2 bg-red-100 text-red-700 font-semibold px-3 py-1.5 rounded-lg hover:bg-red-200 transition-colors border border-red-200"><XCircleIcon className="w-4 h-4" /> Deny</button>
                    <button onClick={() => { onApprove(order.id); onClose(); }} className="flex items-center gap-2 bg-emerald-100 text-emerald-800 font-semibold px-3 py-1.5 rounded-lg hover:bg-emerald-200 transition-colors border border-emerald-200"><CheckCircleIcon className="w-4 h-4" /> Approve</button>
                  </>
              )}
                      ...
                      <div className="flex items-start space-x-3">
                          ...
                          <div>
                              <p className="text-sm font-medium text-brand-brown/70">Delivery Address</p>
                               <a href={mapsUrl} target="_blank" rel="noopener noreferrer" className="text-base text-brand-brown hover:text-brand-orange hover:underline transition-colors flex items-center gap-1.5" aria-label={`Open Google Maps for address: ${order.deliveryAddress}`}><span>{order.deliveryAddress}</span><ArrowTopRightOnSquareIcon className="w-4 h-4" /></a>
                          </div>
                      </div>
                  )}
              </div>
            ...
            <div>
              ...
              <div className="bg-brand-tan/30 p-3 rounded-lg border border-brand-tan">
                  ...
                  {packages.map((pkg, idx) => (
                      <div key={idx} className="mb-3 border-b border-brand-tan last:mb-0 last:border-0 pb-2 last:pb-0">
                          <p className="font-bold text-brand-orange mb-1">{pkg.name}</p>
                          <ul className="pl-2 space-y-1">
                              {pkg.items.map((item, i) => (
                                  <li key={i} className="flex justify-between text-sm">
                                      <span className="font-medium text-brand-brown">{item.name.replace('Full ', '')}</span>
                  ...
                  {/* Loose Items (Extras) */}
                  {looseItems.length > 0 && (
                      <div className={packages.length > 0 ? "pt-2 border-t border-dashed border-brand-tan" : ""}>
                          {packages.length > 0 && <p className="font-bold text-brand-brown text-sm mb-1">Extras</p>}
                          <ul className="divide-y divide-brand-tan/50">
                              {looseItems.map((item, index) => (
                                  <li key={index} className="flex justify-between py-1.5 text-sm">
            ...
            <div className="bg-brand-tan/40 border border-brand-tan p-4 rounded-lg">
              ...
              {onDeductInventory && order.followUpStatus !== FollowUpStatus.COMPLETED && order.approvalStatus === ApprovalStatus.APPROVED && (
                  <div className="mb-4 p-3 bg-white rounded border border-gray-200 flex items-center justify-between">
                      ...
                      <button onClick={handleDeductInventoryClick} disabled={loadingAction !== null} className="flex items-center gap-2 bg-green-600 text-white px-3 py-1.5 rounded shadow-sm text-sm font-medium hover:bg-green-700 transition-colors disabled:bg-gray-400">
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-end border-t border-gray-200 pt-4">
                <div className="flex-grow">
                  ...
                  <select id="followUpStatus" value={localStatus} onChange={handleStatusChange} className="w-full rounded-md border-gray-300 shadow-sm focus:border-brand-orange focus:ring-brand-orange bg-white text-brand-brown">
                ...
                <div className="flex-grow w-full sm:w-auto flex flex-col gap-2">
                   <div className="flex justify-end">
                       <label className="flex items-center text-xs text-gray-600 cursor-pointer select-none">
                           <input type="checkbox" checked={useAi} onChange={(e) => setUseAi(e.target.checked)} className="mr-1.5 rounded text-brand-orange focus:ring-brand-orange border-gray-300" />
                           Use AI Generation (Ignore Template)
                       </label>
                   </div>
                   <button onClick={handleGenerateMessage} disabled={loadingAction === 'message'} className="w-full flex items-center justify-center gap-2 bg-brand-orange text-white font-semibold px-4 py-2 rounded-lg shadow-md hover:bg-opacity-90 transition-colors duration-200 disabled:bg-brand-orange/50">
              ...
              <div className="mt-4">
                ...
                <textarea id="message" value={generatedMessage} onChange={(e) => setGeneratedMessage(e.target.value)} placeholder="Draft a message here or click the button above to auto-generate..." className="w-full h-32 p-2 border border-gray-300 rounded-md bg-white focus:ring-brand-orange focus:border-brand-orange" />
                  ...
                  <div className="mt-2 flex justify-end items-start gap-3 flex-wrap">
                    {order.contactMethod === 'Instagram' && (
                        <div className="text-right">
                            <button onClick={handleSendInstagram} className="inline-flex items-center justify-center gap-2 text-white font-semibold px-4 py-2 rounded-lg shadow-md transition-all duration-200 bg-gradient-to-r from-purple-500 to-pink-500 hover:opacity-90" title="Copy message and open Instagram DMs">
                                <InstagramIcon className="w-5 h-5" /> {copiedState === 'instagram' ? 'Copied!' : 'Follow-up on Instagram'}
                            </button>
                            <p className="text-xs text-gray-500 mt-1">Copies message and opens DMs.</p>
                        </div>
                    )}
                    {order.contactMethod === 'Facebook' && (
                        <div className="text-right">
                            <button onClick={handleSendFacebook} className="inline-flex items-center justify-center gap-2 text-white font-semibold px-4 py-2 rounded-lg shadow-md transition-all duration-200 bg-blue-600 hover:bg-blue-700" title="Copy message and open Facebook Messenger">
                                <FacebookIcon className="w-5 h-5" /> {copiedState === 'facebook' ? 'Copied!' : 'Follow-up on Facebook'}
                            </button>
                            <p className="text-xs text-gray-500 mt-1">Copies message and opens Messenger.</p>
                        </div>
                    )}
                    {order.phoneNumber && (
                        <div className="text-right">
                            <button onClick={handleSendText} className="inline-flex items-center justify-center gap-2 text-white font-semibold px-4 py-2 rounded-lg shadow-md transition-all duration-200 bg-emerald-600 hover:bg-emerald-700" title="Send message using your device's SMS app">
                                <PaperAirplaneIcon className="w-5 h-5" /> Send via Text
                            </button>
                            <p className="text-xs text-gray-500 mt-1">Opens your messaging app.</p>
                        </div>
                    )}
                  </div>
                )}
