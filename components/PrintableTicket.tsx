...
export default function PrintableTicket({ order }: PrintableTicketProps) {
  ...
  return (
    <div className="printable-ticket bg-white text-black p-1.5 font-mono text-[9pt] leading-snug h-full flex flex-col">
      {/* Header section */}
      <div className="text-center font-bold text-[10pt] mb-1">
        <p className="break-all">{order.customerName}</p>
        <p>{order.pickupDate} @ {order.pickupTime}</p>
      </div>
      
      {/* Delivery information */}
      {order.deliveryRequired && (
        <div className="border-t border-black border-dashed pt-1 mb-1">
      ...
      <div className="border-y border-black border-dashed py-1 flex-grow">
