...
import { PlusIcon, TrashIcon, CheckCircleIcon, ArrowTopRightOnSquareIcon } from './icons/Icons';
...
export default function CustomerOrderPage({ empanadaFlavors, fullSizeEmpanadaFlavors }: CustomerOrderPageProps) {
    ...
    const [email, setEmail] = useState(''); // Added email for public form
    ...
    useEffect(() => {
        ...
        const salsaTotal = salsaItems.reduce((sum, item) => {
            if (item.checked) {
                return sum + (item.quantity * SALSA_PRICES[item.size]);
            }
            return sum;
        }, 0);
        // Note: Delivery fee is not calculated here as it usually depends on distance/admin discretion
        setEstimatedTotal(miniTotal + fullSizeTotal + salsaTotal);
    }, [miniItems, fullSizeItems, salsaItems]);
    ...
    const addItem = (type: 'mini' | 'full') => {
        const newItem: FormOrderItem = type === 'mini' 
            ? { name: empanadaFlavors[0], quantity: 12 } // Default to dozen for customers? or 1
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const newOrder: Order = {
                id: Date.now().toString(), // Temporary ID, Firestore will likely overwrite or we use this
                customerName,
                phoneNumber,
                contactMethod: email ? `Website (Email: ${email})` : 'Website Form', // Embed email in contact method for now
                pickupDate: formattedDate,
                pickupTime: formattedTime,
                items: finalItems,
                totalMini: miniItems.reduce((acc, i) => acc + i.quantity, 0),
                totalFullSize: fullSizeItems.reduce((acc, i) => acc + i.quantity, 0),
                amountCharged: estimatedTotal, // Base total, admin adds delivery fee
                amountCollected: 0,
                deliveryRequired,
                deliveryFee: 0, // Admin determines this
                deliveryAddress: deliveryRequired ? deliveryAddress : null,
                paymentStatus: PaymentStatus.PENDING,
                paymentMethod: null,
                followUpStatus: FollowUpStatus.NEEDED,
                specialInstructions: specialInstructions || null,
                approvalStatus: ApprovalStatus.PENDING // Important: Goes to Pending list
            };
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <section className="bg-white p-6 rounded-xl shadow-sm border border-brand-tan">
                        ...
                        <div className="mb-6">
                            <div className="flex justify-between items-center mb-2">
                                <h4 className="font-medium text-brand-brown">Mini Empanadas (${MINI_EMPANADA_PRICE.toFixed(2)})</h4>
                                <button type="button" onClick={() => addItem('mini')} className="text-sm text-brand-orange font-semibold hover:text-brand-brown flex items-center gap-1">
                                    <PlusIcon className="w-4 h-4" /> Add Flavor
                                </button>
                            </div>
                            <div className="space-y-3">
                                {miniItems.map((item, idx) => (
                                    <div key={idx} className="flex gap-3 items-center bg-brand-tan/20 p-2 rounded-lg">
                                        <select 
                                            value={item.name} onChange={e => handleItemChange('mini', idx, 'name', e.target.value)}
                                            className="flex-grow rounded-md border-gray-300 text-sm focus:ring-brand-orange focus:border-brand-orange"
                                        >
                                            {empanadaFlavors.map(f => <option key={f} value={f}>{f}</option>)}
                                        </select>
                                        <input 
                                            type="number" min="1" value={item.quantity} onChange={e => handleItemChange('mini', idx, 'quantity', parseInt(e.target.value) || 0)}
                                            className="w-20 rounded-md border-gray-300 text-sm focus:ring-brand-orange focus:border-brand-orange"
                                        />
                                        <button type="button" onClick={() => removeItem('mini', idx)} className="text-gray-400 hover:text-red-500">
                        ...
                        <div className="mb-6">
                            <div className="flex justify-between items-center mb-2">
                                <h4 className="font-medium text-brand-brown">Full-Size Empanadas (${FULL_SIZE_EMPANADA_PRICE.toFixed(2)})</h4>
                                <button type="button" onClick={() => addItem('full')} className="text-sm text-brand-orange font-semibold hover:text-brand-brown flex items-center gap-1">
                                    <PlusIcon className="w-4 h-4" /> Add Flavor
                                </button>
                            </div>
                            <div className="space-y-3">
                                {fullSizeItems.map((item, idx) => (
                                    <div key={idx} className="flex gap-3 items-center bg-brand-tan/20 p-2 rounded-lg">
                                        <select 
                                            value={item.name} onChange={e => handleItemChange('full', idx, 'name', e.target.value)}
                                            className="flex-grow rounded-md border-gray-300 text-sm focus:ring-brand-orange focus:border-brand-orange"
                                        >
                                            {fullSizeEmpanadaFlavors.map(f => <option key={f} value={f}>{f}</option>)}
                                        </select>
                                        <input 
                                            type="number" min="1" value={item.quantity} onChange={e => handleItemChange('full', idx, 'quantity', parseInt(e.target.value) || 0)}
                                            className="w-20 rounded-md border-gray-300 text-sm focus:ring-brand-orange focus:border-brand-orange"
                                        />
                                        <button type="button" onClick={() => removeItem('full', idx)} className="text-gray-400 hover:text-red-500">
                                            <TrashIcon className="w-5 h-5" />
                                        </button>
                                    </div>
                                ))}
                                ...
                                {salsaItems.map((salsa, idx) => (
                                    <div key={salsa.name} className="flex items-center justify-between bg-gray-50 p-3 rounded-lg border border-gray-200">
                                        <div className="flex items-center">
                                            <input 
                                                type="checkbox" 
                                                checked={salsa.checked} onChange={e => handleSalsaChange(idx, 'checked', e.target.checked)}
                                                className="h-5 w-5 text-brand-orange rounded focus:ring-brand-orange border-gray-300"
                                            />
                                        ...
                                        {salsa.checked && (
                                            <div className="flex gap-2">
                                                 <select 
                                                    value={salsa.size} onChange={e => handleSalsaChange(idx, 'size', e.target.value)}
                                                    className="rounded-md border-gray-300 text-xs focus:ring-brand-orange focus:border-brand-orange py-1"
                                                >
                                                    <option value="Small (4oz)">Small ($2)</option>
                                                    <option value="Large (8oz)">Large ($4)</option>
                                                </select>
                                                <input 
                                                    type="number" min="1" value={salsa.quantity} onChange={e => handleSalsaChange(idx, 'quantity', parseInt(e.target.value))}
                                                    className="w-16 rounded-md border-gray-300 text-xs focus:ring-brand-orange focus:border-brand-orange py-1"
                                                />
                                            </div>
                                        )}
                                    </div>
                    ...
                    <section className="bg-white p-6 rounded-xl shadow-sm border border-brand-tan">
                         ...
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                             ...
                             <div>
                                ...
                                <input 
                                    type="time" required value={pickupTime} onChange={e => setPickupTime(e.target.value)}
                                    className="w-full rounded-lg border-gray-300 focus:ring-brand-orange focus:border-brand-orange"
                                />
                            </div>
                        </div>
                        ...
                        <div className="flex items-start gap-3 mb-4 p-3 bg-blue-50 rounded-lg">
                            <input 
                                type="checkbox" id="delivery" checked={deliveryRequired} onChange={e => setDeliveryRequired(e.target.checked)}
                                className="mt-1 h-5 w-5 text-brand-orange rounded focus:ring-brand-orange border-gray-300"
                            />
                            <div>
                        ...
                        {deliveryRequired && (
                            <div className="animate-fade-in">
                                ...
                                <input 
                                    type="text" required={deliveryRequired} value={deliveryAddress} onChange={e => setDeliveryAddress(e.target.value)}
                                    className="w-full rounded-lg border-gray-300 focus:ring-brand-orange focus:border-brand-orange"
                                    placeholder="123 Main St, Townsville"
                                />
                            </div>
                        )}
                        ...
                        <div className="mt-4">
                            ...
                            <textarea 
                                ...
                                className="w-full rounded-lg border-gray-300 focus:ring-brand-orange focus:border-brand-orange"
                                placeholder="Allergies, gate codes, or specific requests..."
                            />
                        </div>
                    </section>
                    ...
                    <div className="bg-brand-brown text-brand-cream p-6 rounded-xl shadow-lg flex flex-col sm:flex-row justify-between items-center gap-4">
                        ...
                        <button 
                            ...
                            className="w-full sm:w-auto bg-brand-orange text-white font-bold text-lg px-8 py-3 rounded-lg hover:bg-brand-orange/90 transition-all shadow-md disabled:bg-gray-500 disabled:cursor-not-allowed"
                        >
