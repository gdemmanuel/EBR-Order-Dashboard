...
import { XMarkIcon, TrashIcon, ArrowUturnLeftIcon } from './icons/Icons';
...
export default function TrashBinModal({ onClose }: TrashBinModalProps) {
    ...
    useEffect(() => {
        ...
        const unsubscribe = subscribeToDeletedOrders(setDeletedOrders);
                    ...
                    {deletedOrders.length === 0 ? (
                        ...
                        <ul className="space-y-3">
                            {deletedOrders.map(order => {
                                ...
                                return (
                                    <li key={order.id} className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                                        <div>
                                        ...
                                        <button 
                                            ...
                                            disabled={isLoading}
                                            className="flex items-center gap-2 px-3 py-2 bg-green-50 text-green-700 font-medium rounded hover:bg-green-100 transition-colors border border-green-200 text-sm"
                                        >
