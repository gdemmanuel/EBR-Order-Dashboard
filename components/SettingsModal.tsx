...
export default function SettingsModal({ settings, onClose }: SettingsModalProps) {
    ...
    // Employees (Directly from props to ensure sync, or local state if modifying list)
    // We use local state to reflect additions immediately
                ...
                <div className="space-y-6">
                    <div className="bg-white p-5 rounded-lg border border-brand-tan shadow-sm">
                        ...
                        <div className={`grid grid-cols-1 md:grid-cols-5 gap-3 mb-4 p-4 rounded border items-end transition-colors ${editingEmployeeId ? 'bg-amber-50 border-amber-200' : 'bg-gray-50 border-gray-200'}`}>
                            <div className="md:col-span-2"><label className="block text-xs font-bold text-gray-700 mb-1">Name</label><input type="text" value={newEmpName} onChange={e => setNewEmpName(e.target.value)} className="w-full rounded border-gray-300 text-sm p-1.5" placeholder="Employee Name"/></div>
                            <div><label className="block text-xs font-bold text-gray-700 mb-1">Wage ($/hr)</label><input type="number" step="0.50" value={newEmpRate} onChange={e => setNewEmpRate(e.target.value)} className="w-full rounded border-gray-300 text-sm p-1.5" placeholder="15.00"/></div>
                            <div><label className="block text-xs font-bold text-gray-700 mb-1">Mini/Hr</label><input type="number" value={newEmpMini} onChange={e => setNewEmpMini(e.target.value)} className="w-full rounded border-gray-300 text-sm p-1.5" placeholder="40"/></div>
                            <div><label className="block text-xs font-bold text-gray-700 mb-1">Full/Hr</label><input type="number" value={newEmpFull} onChange={e => setNewEmpFull(e.target.value)} className="w-full rounded border-gray-300 text-sm p-1.5" placeholder="25"/></div>
                            <div className="md:col-span-5 flex gap-2">
                                ...
                                <button onClick={handleAddOrUpdateEmployee} className={`w-full text-white font-bold py-2 rounded text-sm shadow-sm hover:bg-opacity-90 flex items-center justify-center gap-2 ${editingEmployeeId ? 'bg-amber-500' : 'bg-brand-orange'}`}>{editingEmployeeId ? <><CheckCircleIcon className="w-4 h-4"/> Update Employee</> : <><PlusIcon className="w-4 h-4"/> Add Employee</>}</button>
                            </div>
                        </div>
                            ...
                            {employees.map(emp => (
                                <div key={emp.id} className={`flex justify-between items-center p-3 bg-white border rounded-lg shadow-sm ${editingEmployeeId === emp.id ? 'border-amber-500 ring-1 ring-amber-500' : 'border-gray-200'}`}>
                                    <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold text-xs">{emp.name.substring(0,2).toUpperCase()}</div>
                                        <div><p className="font-bold text-brand-brown text-sm">{emp.name}</p><p className="text-xs text-gray-500">${emp.hourlyRate.toFixed(2)}/hr • Speed: {emp.speedMini} (m), {emp.speedFull} (f)</p></div>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <button onClick={() => handleEditEmployee(emp)} className="text-gray-400 hover:text-brand-brown p-2 hover:bg-gray-100 rounded-full transition-colors"><PencilIcon className="w-4 h-4"/></button>
                                        <button onClick={() => handleDeleteEmployee(emp.id)} className="text-red-400 hover:text-red-600 p-2 hover:bg-red-50 rounded-full transition-colors"><TrashIcon className="w-4 h-4"/></button>
                                    </div>
                                </div>
                            ))}
                            {employees.length === 0 && (<div className="text-center py-8 text-gray-400 italic border-2 border-dashed border-gray-200 rounded-lg">No employees added yet.</div>)}
                        </div>
                    </div>
                </div>
            );
