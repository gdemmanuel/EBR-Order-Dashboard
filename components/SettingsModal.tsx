                        ...
                        <div className="space-y-8">
                            {/* ... Packages Content ... */}
                             ...
                             <div className="bg-gray-50 p-5 rounded-lg border border-gray-200 shadow-sm">
                                <h3 className="font-bold text-gray-600 text-sm mb-4 uppercase tracking-wide">Fallback / Volume Pricing (Single Item)</h3>
                                
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    {/* Mini Pricing */}
                                    <div>
                                        <div className="flex items-center justify-between mb-2">
                                            <label className="block text-sm font-bold text-brand-brown">Mini Base Price</label>
                                            <div className="relative w-24">
                                                <span className="absolute inset-y-0 left-0 pl-2 flex items-center text-gray-500 text-xs">$</span>
                                                <input type="number" step="0.01" value={pricing.mini.basePrice} onChange={(e) => setPricing({...pricing, mini: { ...pricing.mini, basePrice: parseFloat(e.target.value) || 0 }})} className="block w-full rounded-md border-gray-300 pl-5 text-sm" />
                                            </div>
                                        </div>
                                        
                                        <div className="bg-white border border-gray-200 rounded p-3">
                                            <p className="text-xs font-semibold text-gray-500 mb-2">Volume Tiers (Discount by Qty)</p>
                                            <div className="space-y-2 mb-2">
                                                {(pricing.mini.tiers || []).map((tier, idx) => (
                                                    <div key={idx} className="flex justify-between items-center text-xs bg-gray-50 p-1 rounded">
                                                        <span>{tier.minQuantity}+ items</span>
                                                        <div className="flex items-center gap-2">
                                                            <span className="font-bold">${tier.price.toFixed(2)}</span>
                                                            <button onClick={() => removeTier('mini', tier.minQuantity)} className="text-red-400 hover:text-red-600"><XMarkIcon className="w-3 h-3"/></button>
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
                                            <div className="flex gap-2 items-center">
                                                <input type="number" placeholder="Min Qty" value={newTier.type === 'mini' ? newTier.minQty : ''} onChange={e => setNewTier({ type: 'mini', minQty: e.target.value, price: newTier.type === 'mini' ? newTier.price : '' })} className="w-20 text-xs rounded border-gray-300"/>
                                                <input type="number" placeholder="Price" value={newTier.type === 'mini' ? newTier.price : ''} onChange={e => setNewTier({ type: 'mini', minQty: newTier.type === 'mini' ? newTier.minQty : '', price: e.target.value })} className="w-20 text-xs rounded border-gray-300"/>
                                                <button onClick={addTier} className="text-xs bg-brand-orange text-white px-2 py-1 rounded">Add</button>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        <div className="flex items-center justify-between mb-2">
                                            <label className="block text-sm font-bold text-brand-brown">Full Base Price</label>
                                            <div className="relative w-24">
                                                <span className="absolute inset-y-0 left-0 pl-2 flex items-center text-gray-500 text-xs">$</span>
                                                <input type="number" step="0.01" value={pricing.full.basePrice} onChange={(e) => setPricing({...pricing, full: { ...pricing.full, basePrice: parseFloat(e.target.value) || 0 }})} className="block w-full rounded-md border-gray-300 pl-5 text-sm" />
                                            </div>
                                        </div>
                                            ...
                                            <div className="space-y-2 mb-2">
                                                {(pricing.full.tiers || []).map((tier, idx) => (
                                                    <div key={idx} className="flex justify-between items-center text-xs bg-gray-50 p-1 rounded">
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            <span className="font-bold">${tier.price.toFixed(2)}</span>
                                                            <button onClick={() => removeTier('full', tier.minQuantity)} className="text-red-400 hover:text-red-600"><XMarkIcon className="w-3 h-3"/></button>
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
                                            <div className="flex gap-2 items-center">
                                                <input type="number" placeholder="Min Qty" value={newTier.type === 'full' ? newTier.minQty : ''} onChange={e => setNewTier({ type: 'full', minQty: e.target.value, price: newTier.type === 'full' ? newTier.price : '' })} className="w-20 text-xs rounded border-gray-300"/>
                                                <input type="number" placeholder="Price" value={newTier.type === 'full' ? newTier.price : ''} onChange={e => setNewTier({ type: 'full', minQty: newTier.type === 'full' ? newTier.minQty : '', price: e.target.value })} className="w-20 text-xs rounded border-gray-300"/>
                                                <button onClick={addTier} className="text-xs bg-brand-orange text-white px-2 py-1 rounded">Add</button>
                                            </div>
                                        </div>
                                    </div>
