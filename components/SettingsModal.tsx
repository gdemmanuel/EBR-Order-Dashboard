                    ...
                    <div className="flex-grow overflow-y-auto p-4 md:p-8 bg-white">
                        {activeTab === 'menu' && (
                            <div className="grid grid-cols-1 gap-8 max-w-4xl">
                                ...
                                <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                                    <h3 className="font-bold text-brand-brown mb-2">Salsas & Extras</h3><div className="flex gap-2 mb-4"><input type="text" placeholder="Name" value={newSalsaName} onChange={(e) => setNewSalsaName(e.target.value)} className="flex-grow rounded-md border-gray-300 shadow-sm focus:ring-brand-orange focus:border-brand-orange text-sm"/><input type="number" placeholder="Price" value={newSalsaPrice} onChange={(e) => setNewSalsaPrice(e.target.value)} className="w-24 rounded-md border-gray-300 shadow-sm focus:ring-brand-orange focus:border-brand-orange text-sm"/><button onClick={addSalsa} className="bg-brand-orange text-white px-3 rounded-md hover:bg-opacity-90"><PlusIcon className="w-5 h-5" /></button></div><div className="space-y-2">{pricing.salsas?.map(s => (<div key={s.id} className="bg-white p-2 rounded shadow-sm flex justify-between items-center text-sm"><div><span className={`font-medium ${!s.visible ? 'text-gray-400 line-through' : ''}`}>{s.name}</span></div><div className="flex items-center gap-2"><input type="number" value={s.price} onChange={(e) => updateSalsaPrice(s.id, e.target.value)} className="w-16 text-xs border-gray-200 rounded"/><button onClick={() => toggleSalsaVisibility(s.id)} className={`text-xs px-2 py-1 rounded ${s.visible ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-500'}`}>{s.visible ? 'Visible' : 'Hidden'}</button><button onClick={() => removeSalsa(s.id)} className="text-red-400 hover:text-red-600"><TrashIcon className="w-4 h-4" /></button></div></div>))}</div>
                                </div>
                            </div>
                        )}
