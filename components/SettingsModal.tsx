...
import { AppSettings, updateSettingsInDb, saveEmployeeToDb, deleteEmployeeFromDb, subscribeToEmployees } from '../services/dbService';
import { PricingSettings, MenuPackage, Flavor, Employee } from '../types';
...
export default function SettingsModal({ settings, onClose }: SettingsModalProps) {
    ...
    const [prepSettings, setPrepSettings] = useState<AppSettings['prepSettings']>(settings.prepSettings || { lbsPer20: {}, fullSizeMultiplier: 2.0, discosPer: { mini: 1, full: 1 }, discoPackSize: { mini: 10, full: 10 }, productionRates: { mini: 40, full: 25 } });
    const [scheduling, setScheduling] = useState<AppSettings['scheduling']>(settings.scheduling || { enabled: true, intervalMinutes: 15, startTime: "09:00", endTime: "17:00", blockedDates: [], closedDays: [], dateOverrides: {} });
    ...
    const [newEmployeeColor, setNewEmployeeColor] = useState(PRESET_COLORS[0]);

    // ... (Handlers for existing tabs omitted for brevity but preserved in output logic by using same file structure) ...
    // Re-implementing existing handlers to ensure full file validity
            ...
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col border border-brand-tan">
                ...
                <div className="flex flex-wrap border-b border-gray-200">
                    {['menu', 'pricing', 'scheduling', 'prep', 'costs', 'expenses', 'employees'].map((tab) => (
                        ...
                        >
                            {tab === 'expenses' ? 'Exp. Categories' : tab === 'costs' ? 'Inventory & Costs' : tab === 'prep' ? 'Prep' : tab === 'employees' ? 'Employees' : tab}
                        </button>
                    ))}
                </div>

                <div className="overflow-y-auto p-6 flex-grow">
                    {/* ... Previous Tabs (Menu, Pricing, Scheduling, Prep, Costs, Expenses) ... */}
                    ...
                    {activeTab === 'employees' && (
                        <div className="space-y-8">
                            <div className="bg-white p-5 rounded-lg border border-brand-tan shadow-sm">
                                <h3 className="font-bold text-brand-brown text-lg mb-4 border-b pb-2">Staff Management</h3>
                                <div className="flex flex-wrap gap-4 mb-6 items-end">
                                    <div className="flex-grow">
                                        <label className="block text-xs font-medium text-gray-700 mb-1">Name</label>
                                        <input type="text" value={newEmployeeName} onChange={e => setNewEmployeeName(e.target.value)} className="w-full text-sm rounded border-gray-300" placeholder="John Doe"/>
                                    </div>
                                    <div className="w-32">
                                        <label className="block text-xs font-medium text-gray-700 mb-1">Hourly Rate ($)</label>
                                        <input type="number" value={newEmployeeRate} onChange={e => setNewEmployeeRate(e.target.value)} className="w-full text-sm rounded border-gray-300" placeholder="15.00"/>
                                    </div>
                                    <div>
                                        ...
                                        <div className="flex gap-1">
                                            {PRESET_COLORS.map(c => (
                                                <button 
                                                    key={c} 
                                                    onClick={() => setNewEmployeeColor(c)}
                                                    className={`w-6 h-6 rounded-full border-2 ${newEmployeeColor === c ? 'border-gray-600 scale-110' : 'border-white'}`}
                                                    style={{ backgroundColor: c }}
                                                />
                                            ))}
                                        </div>
                                    </div>
                                    <button onClick={handleAddEmployee} className="bg-brand-orange text-white px-4 py-2 rounded text-sm font-semibold">Add Staff</button>
                                </div>

                                <div className="space-y-3">
                                    {employees.map(emp => (
                                        <div key={emp.id} className="flex justify-between items-center p-3 bg-gray-50 rounded border border-gray-200">
                                            <div className="flex items-center gap-3">
                                                <div className="w-4 h-4 rounded-full" style={{ backgroundColor: emp.color }}></div>
                                                <div>
                                                    <p className="font-bold text-brand-brown">{emp.name}</p>
                                                    <p className="text-xs text-gray-500">${emp.hourlyRate.toFixed(2)} / hr</p>
                                                </div>
                                            </div>
                                            <button onClick={() => handleDeleteEmployee(emp.id)} className="text-red-400 hover:text-red-600"><TrashIcon className="w-4 h-4"/></button>
                                        </div>
                                    ))}
                                    {employees.length === 0 && <p className="text-gray-500 text-sm italic">No employees added.</p>}
                                </div>
                            </div>
                        </div>
                    )}
